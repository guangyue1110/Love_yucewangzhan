...
// 首先定义类型
type CategoryKey = '基础信息' | '个性特征' | '生活习惯' | '伴侣期望';
...
export default function ResultPage() {
  ...
  const calculateMatchScore = () => {
    ...
    answers.forEach(answer => {
      ...
      categoryScores[question.category as CategoryKey] += score
    })
    ...
    Object.keys(categoryScores).forEach(category => {
      ...
      if (categoryQuestions.length > 0) {
        categoryScores[category as CategoryKey] = categoryScores[category as CategoryKey] / categoryQuestions.length
      }
    })

    // 计算加权总分
    const totalScore = Object.entries(categoryScores).reduce((total, [category, score]) => {
      return total + score * categoryWeights[category as CategoryKey]
    }, 0)
            ...
            <div className="text-center mb-10">
              ...
              >
                <span className="text-8xl md:text-9xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500">
                  {Math.round(matchResult.score as number)}%
                </span>
                {/* 添加闪烁的星星 */}
                <motion.div 
                  animate={{ 
