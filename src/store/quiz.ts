...
const useQuizStore = create<QuizState>((set) => ({
  ...
  submitMultipleChoice: () => set((state) => {
    const currentQuestion = state.questions[state.currentQuestion]
    if (currentQuestion.type !== '多选题' || state.selectedOptions.length === 0) {
      return state
    }
    ...
    return {
      answers: [...state.answers.filter(a => a.questionId !== currentQuestion.id), answer],
      currentQuestion: state.currentQuestion < state.questions.length - 1 
